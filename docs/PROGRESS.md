...
- [x] GH #17 / ISSUE-016：`PYTHONPATH=src python3 -m pytest --collect-only tests/test_ex_payloads.py` 收集 69 个测试，退出码 0
- [x] GH #17 / ISSUE-016：`PYTHONPATH=src python3 -m pytest -q tests/test_skeleton_imports.py tests/test_ex_payloads.py` 退出码 0；当前环境未安装 console scripts，既有 entrypoint 检查按测试逻辑 skip
...
- [ ] GH #17 / ISSUE-016：`pytest --collect-only tests/test_ex_payloads.py` 在当前环境返回 `command not found`；已使用 `python3 -m pytest` 等价验证
...
| 日期 | 变更 | 来源 |
|------|------|------|
| 2026-04-15 | 完成 GH #17 / ISSUE-016 Ex-0~Ex-3 Pydantic 校验单元测试，记录沙箱验证结果 | GH #17 |
